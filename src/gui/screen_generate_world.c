...
screen_t screen_generate_world_create(screen_t *parent) {
    ...
    screen.on_open = screen_generate_world_on_open;
    screen.on_button_clicked = screen_generate_world_on_button_clicked;
    screen.render = screen_generate_world_render;

    return screen;
}

void screen_generate_world_on_open(struct screen_s *proto_screen) {
...
void screen_generate_world_on_button_clicked(struct screen_s *proto_screen, button_t *button) {
    screen_t *screen = (screen_t *)proto_screen;
    if(button->id == 200) {
        minecraft_set_current_screen(screen->minecraft, (screen_t *)screen->parent);
        return;
    }
    minecraft_regenerate_world(screen->minecraft, button->id);
    minecraft_set_current_screen(screen->minecraft, NULL);
}

void screen_generate_world_render(struct screen_s *proto_screen, int mouse_x, int mouse_y, float partial_tick) {
    screen_t *screen = (screen_t *)proto_screen;
    gui_fill_gradient(0, 0, screen->width, screen->height, 0x05050060, 0x303060A0);
    gui_draw_centered_string(screen->font, "Generate new world", screen->width / 2, 40, 0xFFFFFFFF);
