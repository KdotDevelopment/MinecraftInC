...
screen_t screen_load_world_create(screen_t *parent) {
    ...
    screen.on_open = screen_load_world_on_open;
    screen.on_button_clicked = screen_load_world_on_button_clicked;
    screen.render = screen_load_world_render;
...
void screen_load_world_on_button_clicked(struct screen_s *proto_screen, button_t *button) {
    ...
    if(button->id < 5) screen_load_world_open_world(proto_screen, button->id);
...
void screen_load_world_open_world(struct screen_s *proto_screen, int world) {
...
void screen_load_world_open_world_from_file(struct screen_s *proto_screen, char *file) {
    minecraft_set_current_screen(((screen_t *)proto_screen)->minecraft, (screen_t *)((screen_t *)proto_screen)->parent);
    //minecraft_grab_mouse(((screen_t *)proto_screen)->minecraft);
}
