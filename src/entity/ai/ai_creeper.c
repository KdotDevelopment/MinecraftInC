...
ai_t ai_creeper_create(struct mob_s *mob) {
    ai_t ai = ai_basic_attack_create(mob->world, mob);
...
void ai_creeper_before_removal(ai_t *ai) {
    float radius = 4.0;
    world_explode(ai->mob->world, &ai->mob->entity, ai->mob->x, ai->mob->y, ai->mob->z, radius);
    ...
    for(int i = 0; i < 500; i++) {
        ...
        *particle = particle_terrain_create(ai->mob->world, ai->mob->x + x, ai->mob->y + y, ai->mob->z + z, xd, yd, zd, &block_list[blocks.leaves.id]);
        particles_spawn_particle(ai->mob->world->particles, particle);
    }
}